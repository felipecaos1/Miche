...
function Configurar_productos() {
    const [lista_mp, setLista] = useState([]);
    React.useEffect(() => {
        actualizar_lista();
    }, [])
    ...
    const actualizar_lista = async () => {
        const temp = [];
        await fetch(`http://localhost:8081/listCongfigMaterias`)
            .then(res => res.json())
            .then(res => {
    ...
    const guardar_producto = () => {
        ...
        fetch(`http://localhost:8081/configurarProducto`, {
            method: 'POST', // or 'PUT'
            body: JSON.stringify(producto_json), // data can be `string` or {object}!
            headers: {
